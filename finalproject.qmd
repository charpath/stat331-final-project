...
First, in order to make fitting the model slightly easier, we're going to condense each country's observations down into one row by taking the median value of the observations and taking the log of our observations.  
...
lr_murders_unemployment <- murders_unemployment |> 
  ...
  mutate(med_murders_per_100000 = log(med_murders_per_100000))
```
...
$\widehat{Median\ Murders\ per\ 100,000}\ =\ 0.0293\ -\ 0.339(Median\ Unemployment)$

The intercept is 0.339, meaning when unemployment rate is at 0%, we expect to have $e^{0.339}$ or 1.4 murders per 100,000. The coefficient of Median Unemployment Rate is 0.0293, so for each 1% unemployment rate increases by in a country, we expect the murders per 100,000 to increase by $e^{0.0293}$ or roughly 1 person per 100,000.
...
The $R^2$ is 0.002 meaning that virtually none of the variability in our observed data is explained by our simulation results. Our simulated data is not a good explanation of our observed data. 
...
simulation |>
  ...
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "Simulated Murder per 100000", y = "Observed Murder per 100000",
...
The relationship between the observed murder rate and simulated murder rate to unemployment looks quite different.
...
for(i in 1:num_simulations){
  ...
  mod2 <- lm(med_murders_per_100000 ~ sim_murder, data = complete_data)
